import { BookCheck, CandlestickChart,Home,List,UsersRound,LogOut, User } from "lucide-react";
// import { UserType } from "../../interfaces";
import { useLocation, useNavigate } from "react-router-dom";
import Cookies from "js-cookie";
import {message} from"antd";
import usersGlobslStore, {UsersStoreType}from "../../store/users-store";
...
function MenuItems() {
    ...
    const {currentUser} : UsersStoreType =  usersGlobslStore() as UsersStoreType;
    ...
    return (
    <div className="bg-gray-200 h-full p-5 w-full">
        <div className="flex flex-col gap-1 mt-5">
            <h1 className="text-2xl font-bold text-info">RAJ
            ...
            <div className="flex flex-col gap-10 mt-20">
                {menuToRender.map((item:any) => (
                    <div className={`cursor-pointer px-5 py-3 rounded flex gap-5 text-sm items-center ${item.isActive ? 'bg-info text-white' : ''}` }
                        key={item.name}
                        onClick={() => {
                            if(item.name==="LogOut") {
                                onLogout();
